...
常见错误码：`MISSING_FIELDS` `INVALID_PRICING` `INVALID_CATEGORY` `CAPTCHA_FAILED` `INVALID_NAME`
...
| `wrangler.toml` | Worker 部署配置（D1/KV 绑定、路由） | ✅ 提交（空占位符） |
| `.dev.vars` | 本地环境变量模板 | ✅ 提交（模板值） |
| `.dev.vars.local` | 本地真实环境变量 | ❌ .gitignore 排除 |
| `.wrangler/` | wrangler 缓存/认证信息 | ❌ .gitignore 排除 |
| `.claude/rules/product-scope.md` | D1 表结构定义 | ✅ 提交 |
